...
class Parser(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.category_ids = self.pool.get('product.category').search(self.cr, self.uid, [
            ('id', 'in', category_ids)], order=categories_order, context=context)
        self.categories = self.pool.get('product.category').browse(
            self.cr, self.uid, self.category_ids, context=context)
    ...
    def get_products(self, category_ids, context=None):
        ...
        template_ids = self.pool.get('product.template').search(
            self.cr, self.uid, [('categ_id', 'in', category_ids)], context=context)
        ...
        # TODO: al pasar la funcion get_products desde el reporte no se ordenan
        # los productos

        product_ids = self.pool.get('product.product').search(self.cr, self.uid, [
            ('product_tmpl_id', 'in', template_ids), ], order=order, context=context)
        print product_ids
