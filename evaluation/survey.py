...
class survey_user_input(osv.Model):
	...
	def compute_score(self, cr, uid, ids, context=None):
		...
		for user_input in self.browse(cr, uid, ids, context=context):
			computed_score = 0
			for question in self.get_questions_of_user_input_survey(user_input):
				user_input_lines = self.get_user_input_lines_of_question(cr, uid, user_input, question, context=context)
				if question.type == 'simple_choice' or question.score_calc_method == 'direct_sum':
					...
					# TODO remove this line if score is calculted correctly
					# pre_score = sum([user_input_line.value_suggested.score for user_input_line in user_input_lines])
	...
	def get_user_input_lines_of_question(self, cr, uid, user_input, question, context=None):
		user_input_lines_obj = self.pool.get('survey.user_input_line')
		user_input_lines_ids = user_input_lines_obj.search(cr, uid,
			[('question_id', '=', question.id), ('user_input_id', '=', user_input.id)],
			context=context)
		return user_input_lines_obj.browse(cr, uid, user_input_lines_ids, context=context)

	def get_questions_of_user_input_survey(self, user_input):
		questions = []
