    ...
    def cancel_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.receipt_id and voucher.receipt_id.state != 'draft':
                self.pool['account.voucher.receipt'].cancel_receipt(
                    cr, uid, [voucher.receipt_id.id], context=context)
        return True
    ...
    def action_cancel_draft(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.receipt_id:
                self.pool['account.voucher.receipt'].action_cancel_draft(
                    cr, uid, [voucher.receipt_id.id], context=context)
        return True
    ...
    def close_receipt(self, cr, uid, ids, context=None):
        ...
        if voucher.receiptbook_id.sequence_type == 'manual' or voucher.receipt_id.origin_voucher_id.id != voucher.id:
            view_ref = self.pool.get('ir.model.data').get_object_reference(
                cr, uid, 'account_voucher_receipt', 'receipt_validate_finish_form')
    ...
    def create_or_add_receipt(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if not record.receipt_id and not context.get('default_receipt_id', False):
                ...
                receipt_id = self.pool['account.voucher.receipt'].create(
                    cr, uid, receipt_vals, context=context)
                res[record.id] = receipt_id
                self.write(
                    cr, uid, record.id, {'receipt_id': receipt_id}, context=context)
            else:
