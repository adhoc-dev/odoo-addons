...
class account_invoice_prices_update(osv.osv_memory):
    ...
    _defaults = {
        'pricelist_id':lambda s, cr, u, c: s.pool.get('res.users').browse(cr, u, u, c).partner_id.property_product_pricelist.id,
    }
    ...
    def update_prices(self, cr, uid, ids, context=None):
        ...
        if not wizard.pricelist_id:
            return {}
        invoice_line = self.pool['account.invoice'].browse(cr, uid ,active_id ,context=context).invoice_line
        for line in invoice_line:
                price = self.pool.get('product.pricelist').price_get(cr, uid, [wizard.pricelist_id.id],
